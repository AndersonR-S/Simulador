...
class PygameWidget(QWidget):
    ...
    def resizeEvent(self, event):
        ...
        self.screen = pygame.Surface((self.width*0.7, self.height*0.8))
...
class Interface(QMainWindow):
    ...
    def arquivo(self):
        ...
        buttons = [
            ...
            ("Visualizar campo elétrico", self.vazio),
        ]
    ...
    def display_charge_data(self, charge_number):
            ...
            if charge is not None:
                ...
                force_label = QLabel()
                force_label.setStyleSheet("font-family: fonte; font-size: 20px; border: none;")
                force_label.setText(f"Vetor de Força: ( {force['total_force'][0]:.2e} , {force['total_force'][1]:.2e} )")
                self.layout_menu_esquerdo.addWidget(force_label)

                force_magnitude_label = QLabel()
                force_magnitude_label.setStyleSheet("font-family: fonte; font-size: 20px; border: none;")
                force_magnitude_label.setText(f"Magnitude: {force['magnitude_total']:.2e}")
